...
<template>
  <button
    type="button"
    class="atc"
    :class="{ 'atc--icon': variant === 'icon', 'atc--block': block }"
    :aria-label="variant === 'icon' ? label : undefined"
    @click="$emit('click', $event)"
  >
    <span class="atc__icon" aria-hidden="true">
      <svg class="atc__svg" viewBox="0 0 19.9815 20" width="20" height="20" xmlns="http://www.w3.org/2000/svg">
        <path
          fill="currentColor"
    ...
    <span v-if="variant === 'default'" class="atc__label">{{ label }}</span>
  </button>
</template>
