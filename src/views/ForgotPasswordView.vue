...
<template>
  <div class="auth-shell">
    ...
    <div class="auth-shell__inner">
      <RouterLink to="/" class="auth-shell__back"><span aria-hidden="true">←</span> Back to store</RouterLink>
      <div class="auth-card">
        ...
        <p class="auth-card__text">
          In a full site you would enter your email and receive a reset link. For this demo you can open the account area directly.
        </p>
        <RouterLink class="auth-card__cta" :to="{ name: 'account' }">View demo account</RouterLink>
        <RouterLink class="auth-card__secondary" to="/login">Return to sign in</RouterLink>
      </div>
    </div>
  </div>
</template>
...
.auth-shell__backdrop {
  ...
  background:
    ...
    linear-gradient(165deg, #000f24 0%, var(--color-dark-blue, #001e40) 100%);
  pointer-events: none;
}
.auth-shell__inner {
  position: relative;
...
.auth-card {
  ...
  box-shadow: 0 24px 48px -12px rgba(0, 14, 36, 0.35);
