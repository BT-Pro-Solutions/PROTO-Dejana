...
onMounted(() => {
  if (!auth.isLoggedIn.value) {
    toast.push({
      message: 'Sign in to save and view your favorites.',
...
const sortBy = ref('saved')

const favoriteProducts = computed(() => {
  const list = orderedFavoriteProducts(favorites.ids.value)
  if (sortBy.value === 'saved') return list
  const v = [...list]
  switch (sortBy.value) {
    case 'price-asc':
      return v.sort((a, b) => a.price - b.price)
    case 'price-desc':
      return v.sort((a, b) => b.price - a.price)
    case 'rating':
      return v.sort((a, b) => b.rating - a.rating)
    case 'name':
      return v.sort((a, b) => a.title.localeCompare(b.title))
    default:
      return v
  }
})
...
<template>
  <div class="page">
    <header class="mast" aria-labelledby="fav-heading">
      <div class="wrap">
        <nav class="crumb" aria-label="Breadcrumb">
          <RouterLink to="/">HOME</RouterLink>
          <img class="crumb__ico" :src="media.icons.breadcrumbSep" width="5" height="7" alt="" />
          <span class="crumb__here">FAVORITES</span>
        </nav>

        <div class="mast__row">
          <div class="mast__copy">
            <h1 id="fav-heading" class="mast__h1">Favorites</h1>
            <p class="mast__lead">
              Items you have saved to your list — {{ count }} {{ count === 1 ? 'item' : 'items' }}.
            </p>
          </div>
          <div v-if="count > 0" class="mast__sort">
            <label class="sort">
    ...
    <div class="sheet">
      <div class="wrap">
        <div v-if="count === 0" class="empty">
          ...
          <p class="empty__text">Tap the heart on any product to save it here.</p>
          <div class="empty__actions">
            <RouterLink to="/new-arrivals" class="empty__btn empty__btn--primary">Browse new arrivals</RouterLink>
            <RouterLink to="/shop" class="empty__btn empty__btn--ghost">Shop by category</RouterLink>
          </div>
        </div>
        ...
        <PlpFiltersShell v-else theme="dark" :active-filter-count="activeFilterCount">
          ...
          <div class="results">
            <div class="grid">
              <div v-for="p in favoriteProducts" :key="p.id" class="grid__cell">
                <ProductCard
                  :product="p"
                  category-grid
                  dark-grid
...
<style scoped>
.page {
  --fav-bg: var(--color-dark-blue);
  --fav-line: rgba(255, 255, 255, 0.12);
  --fav-muted: rgba(255, 255, 255, 0.72);
  --plp-sidebar-w: 268px;
  --plp-layout-gap: 48px;
...
.sheet {
  background: var(--fav-bg);
  padding: 28px 0 0;
  box-shadow: inset 0 1px 0 var(--fav-line);
}
